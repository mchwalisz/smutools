...
This directory contains measurement code to do link measurements between one
sender node and N receiver nodes. There are two variants:
...
robin and whenever a receiver has received a packet destined to itself it sends
...
(statistics) over the USB interface via the printf library (see below).
...
Traces:
  ...
  To read the traces by a node (sender/receiver) you need to start the
  ...
  0     : is an error code and should always be 0


  A receiver will print one line per received packets, 
  ...
  720   : is the sequence number from the received packet
